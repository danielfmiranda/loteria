...
class AppContainer extends Component {
  ...
  getUsersCards = () => {
    for (let i = 0; i < 16; ) {
      let randomNumber = Math.floor(
        Math.random() * this.state.possibleCards.length
      );
      let userAlreadyHaveCardBool = this.state.usersCards.some(
        (item) => this.state.possibleCards[randomNumber].name === item.name
      );
  ...
  getRandomNum = () => {
    if (this.state.nums.size !== 56) {
      let numberInShuffledArray = this.state.ranNums[
        this.state.cardCalledNumber
      ];
      if (numberInShuffledArray === []) {
      } else {
        this.state.cardsCalled.add(
          this.state.possibleCards[numberInShuffledArray]
        );
        //
    ...
    return (
      <div className={"appContainer"}>
        {this.state.introButtonClicked === false ? (
          <IntroPage clickIntroButton={this.clickIntroButton} />
        ) : (
          <GameContainer
